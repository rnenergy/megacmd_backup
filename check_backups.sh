#!/bin/bash
#Import vars
. /root/megacmd_backup/comm_vars.txt
#VM ID (optional)
#VM_ID="100"
...
if [[ "$output" == *"$SUB"* ]];
then 
/usr/local/bin/telegram-send "Бэкапы Proxmox VE на $HOSTNAME успешно залиты на Мега.іо!"
else
/usr/local/bin/telegram-send "ТРЕВОГА!!!! Бэкапы Proxmox VE на $HOSTNAME НЕ ЗАЛИТЫ на Мега.іо!!!!"
fi
